...
def send_vistttt(uid):
    try:
        ...
        if response.status_code == 200:
            ...
            # Handle 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 server errors.
            return (
...
def send_likes(uid):
    try:
        ...
        likes_api_response = requests.get(
            f"https://private-like-api.vercel.app/like?={uid}&server_name=ind&key=Nilay-Ron",
...
def get_info(uid):
    try:
        ...
        info_api_response = requests.get(
            f"https://ffinfo-mu.vercel.app/player-info?uid={uid}&region={region}",
            timeout=15  # Add a timeout to prevent it from hanging
        )
...
[FFD700]👑 NAME : [FFFF00]MIKEY DEAD   
[FFD700]📌 Instagram : [00BFFF]@yh.mikeyy 
[FF1493]╚══════════════════════════╝""", idinv
                            )
                    )
                    senthi = False
...
[FFD700][b][c]thank you for supporting follow ig:[/b]

[1E90FF][b][c] Instagram Name: yh.mikeyy[/b]
[1E90FF][c]@[yh.mikeyy/c]

[FFD700][b][c]Developer: MIKEY [/b]
...
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF]MIKEY DEAD   
[FFD700]⚡ TELEGRAM : [FFFFFF]@MIKEYY.14 
[FFD700]✨ Name on instagram : [FFFFFF]yh.mikeyy❤️  
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </> MIKEY DEAD  ⚡""", uid
                            )
                        )
                    except Exception as e:
                ...
                if "1200" in data.hex()[0:4] and b"/sm" in data:
                    try:
                        ...
                        if len(command_split) > 1:
                            ...
                            spam_count = 70  # You can increase or decrease this value
...
[FFD700]✨ Developer —͟͞͞ </> MIKEY DEAD  ⚡""",
                                    uid
                                )
                            )
...
[FFD700]✨ Developer —͟͞͞ </> MIKEY DEAD  ⚡""",
                ...
                if "1200" in data.hex()[0:4] and b"WELCOME TO [FFFFF00]MIKEY  HERE [ffffff]BOT" in data:
                    pass
                else:
                    ...
                    if "1200" in data.hex()[0:4] and b"/ai" in data:
                        try:
                            ...
                            response = requests.post(
                                f"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",
                                headers=headers,
                                json=payload,
                            )
                            if response.status_code == 200:
...
#ob51rio
                ...
                if '1200' in data.hex()[0:4] and b'/lag' in data:
                    try:
                        ...
                        restart_program()
                if "1200" in data.hex()[0:9] and b"/solo" in data:
                    try:
                        json_result = get_available_room(data.hex()[10:])
                        parsed_data = json.loads(json_result)
                        uid = parsed_data["5"]["data"]["1"]["data"]
                ...
                if "1200" in data.hex()[0:4] and b'!e' in data:
                    try:
                        ...
                        # Command format: !e <target_id1> [target_id2...] <emote_id>
                        command_parts = data.split(b'!e')[1].split(b'(')[0].decode().strip().split()
                        if len(command_parts) < 2:
                            clients.send(self.GenResponsMsg("[C][B][FF0000]Usage: !e <target_id> <emote_id>", uid_sender))
                            continue
                        ...
                        logging.error(f"Error processing !e command: {e}")
                        try:
                            uid_sender = json.loads(get_available_room(data.hex()[10:]))["5"]["data"]["1"]["data"]
                            clients.send(self.GenResponsMsg("[C][B][FF0000]Error processing !e command.", uid_sender))
                        except:
                            pass                
